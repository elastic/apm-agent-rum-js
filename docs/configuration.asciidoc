...
This does not have any influence on the types of errors that are sent to the APM Server. Use this option to report an issue with the agent to us.
...
The distributed tracing is enabled by default, but this configuration can be used to disable it.
...
The agent can only send up to `80` events every `60000` milliseconds (one minute) by default.
