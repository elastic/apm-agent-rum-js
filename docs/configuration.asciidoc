...
[source,js]
----
apm.init({ apmRequest: (requestSpec) => true})
----

Arguments:

* `requestSpec` - This is an object that contains the APM http request details:
...
If this function returns a falsy value the request is discarded.
