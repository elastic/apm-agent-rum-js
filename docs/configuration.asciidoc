...
This option sets the name for the page load transaction. By default, Transaction names for Hard and Soft navigations are 
inferred by the agent based on the current URL. Let's look at the below example
...
The page load transactions for the above URL's would be inferred automatically and transaction names will be categorized
as `/blog/:id` and `/guide/en/*` by the agent. A common pattern to name the transactions would be to use the current
URL(window.location.href). However, it creates too many unique transactions(blog titles, query strings, etc) 
and would be less useful when visualizing the traces in UI.

If the default transaction names are not helpful, please set `pageLoadTransactionName` to something
meaningful that groups transactions under the same categories(blog, guide, etc) and avoid using the current URL.
