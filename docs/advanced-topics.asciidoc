...
URL path tree until the depth of 2 and converts them to wildcard or slugged matches based on the number of digits, special characters,
the mix of upper and lowercase characters in the path.
...
meaningful that groups transactions under the same categories (blog, guide, etc.) and avoid using the current URL at all costs.
