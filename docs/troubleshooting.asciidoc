...
NOTE: `Access-Control-Allow-Origin: *` will not match `localhost` in some browsers (e.g. https://bugs.chromium.org/p/chromium/issues/detail?id=67743[in Chrome]), causing `Script error` messages when the configuration is otherwise correct in local development environments.
